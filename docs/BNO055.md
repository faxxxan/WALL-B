...
- `read_euler()`: Prints only the Euler angles to the console. Currently used for balance via personality module.
- `publish_changed_data()`: Publishes sensor data to the pubsub topic `imu/<name>/data` when significant changes are detected.
