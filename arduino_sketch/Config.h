...
#define PIN_SNX 4 // Servo neck extension
// Left Leg
#define PIN_SLLH 8 // Servo left leg hip
#define PIN_SLLK 9 // Servo left leg knee
#define PIN_SLLA 10 // Servo left leg ankle
// Right Leg
#define PIN_SRLH 5 // Servo right leg hip
#define PIN_SRLK 6 // Servo right leg knee
#define PIN_SRLA 7 // Servo right leg ankle
...
#define SERVO_COUNT 9 // Number of servos to be controlled by ServoEasing
...
int PosMin[SERVO_COUNT] = {0, 0, 5, 0, 0, 20, 60, 30, 0};
int PosMax[SERVO_COUNT] = {180, 180, 165, 180, 180, 180, 120, 150, 90};
int PosSleep[SERVO_COUNT] = {40, 60, 95, 140, 120, 85, PosMax[7], 90, 90};
...
// Starting positions
//0, 3 = HIP
int PosStart[SERVO_COUNT] = {60, 0, 80, 120, 180, 100, 90, 90, 90};

int PosBackpack[SERVO_COUNT] = {30, 5, 130, 130, 120, 160, 90, 90, 90}; // Position legs to support when mounted to backpack
int PosStraight[SERVO_COUNT] = {45, 90, 165, 135, 90, 20, 90, 90, 90}; // straighten legs and point feet to fit in backpack upright

//int PosRest[SERVO_COUNT] = {S1_REST, S2_REST, S3_REST, S4_REST, S5_REST, S6_REST, S7_REST, S8_REST, S9_REST};
int PosRest[SERVO_COUNT] = {60, 0, 165, 120, 180, 20, 90, 90, 90};

int PosConfig[SERVO_COUNT] = {90, 90, 90, 90, 90, 90, 90, 90, 90};

// Poses
int PosStand[SERVO_COUNT] = {45, 70, 80, 135, 110, 100, NOVAL, NOVAL, 90};
int PosLookLeft[SERVO_COUNT] = {NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, 180, 90};
int PosLookRight[SERVO_COUNT] = {NOVAL, NOVAL, NOVAL, NOVAL, NOVAL,  NOVAL, NOVAL, 0, 90};
int PosLookRandom[SERVO_COUNT] = {NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, -1, -1, 90}; // Made random by calling the function moveRandom() if the value is -1
int PosLookRandomRight[SERVO_COUNT] = {NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, -1, 30, 90};
int PosLookRandomLeft[SERVO_COUNT] = {NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, -1, 150, 90};
int PosLookUp[SERVO_COUNT] = {NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, 60, 90, 90};
int PosLookDown[SERVO_COUNT] = {NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, NOVAL, 120, 90, 90};
